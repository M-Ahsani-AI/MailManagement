// frontend/src/components/KategoriList.js
...
export default function KategoriList({
  ...
  const filtered = kategori.filter((k) => {
    ...
    return (
      ...
      k.nama_kategori?.toLowerCase().includes(search) ||
      k.template?.toLowerCase().includes(search) 
    );
  });
      ...
      <table className="w-full table-auto">
        <thead className="bg-gray-100">
          <tr>
            ...
            <th className="px-4 py-3 text-left text-sm font-medium text-gray-700">Detail</th>
          ...
          {loading ? (
            ...
            current.map((k, index) => (
              <tr key={k.kode} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3 text-sm"> {k.kode} </td>
                <td className="px-4 py-3 text-sm"> {k.nama_kategori} </td>
