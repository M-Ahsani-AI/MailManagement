// frontend/src/components/BastList.js
...
export default function BastList({
  ...
  error,
    ...
    const filtered = bast.filter((b) => {
    if (!searchTerm) return true; 
    const search = searchTerm.toLowerCase();
    return (
      b.no_bast?.toLowerCase().includes(search) ||
      b.kategori_kode?.toLowerCase().includes(search) ||
      b.perihal?.toLowerCase().includes(search) ||
      b.tanggal?.toLowerCase().includes(search) 
    );
  });
    ...
    <div className="bg-white rounded-lg shadow overflow-hidden">
      <table className="w-full table-auto">
        <thead className="bg-gray-100">
          <tr>
            <th className="px-4 py-3 text-left text-sm font-medium text-gray-700">No Bast</th>
          ...
          {loading ? (
            ...
            current.map((b, index) => (
              <tr key={b.id} className="hover:bg-gray-50">
                <td className="px-4 py-3 text-sm">{b.no_bast}</td>
                <td className="px-4 py-3 text-sm">{b.kategori_kode}</td>
                <td className="px-4 py-3 text-sm">{b.perihal}</td>
                <td className="px-4 py-3 text-sm">{formatDate(b.tanggal)}</td>
                <td className="px-4 py-3 text-sm flex gap-1">
                                <button
                                    onClick={() => onEdit(b)} // ❌ Diperbaiki dari onEdit(b) menjadi onEdit(s)
